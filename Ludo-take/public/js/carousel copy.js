...
// by counting the card on the carousel and looking the screen width. It almost stop when the mouse is under him.
    ...
    howMuchCarouselElement : document.getElementsByClassName('carousel1__window__card').length,
    ...
    init: function() {
        ...
        for (let carouselNumber = 0; carouselNumber < document.getElementsByClassName('carousel').length; carouselNumber++) {
                console.log('ici');
            carousel.howMuchCarouselElement[carouselNumber] = document.getElementsByClassName('carousel'+carouselNumber+'__window__card').length;
            carousel.next[carouselNumber] = document.querySelector('.carousel'+carouselNumber+'__arrow--next');
            carousel.previous[carouselNumber] = document.querySelector('.carousel'+carouselNumber+'__arrow--previous');
            carousel.slider[carouselNumber] = document.querySelector('.carousel'+carouselNumber+'__window__slider');

            carousel.selectSlidePerCent();
            carousel.selectTranslatePerCent();
            carousel.lastElementBeforeReverse = carousel.howMuchCarouselElement[carouselNumber]-carousel.howMuchActiveCarrouselEllement+1;
            carousel.previous[carouselNumber].addEventListener('click', carousel.handleCarouselPrevious);
            carousel.next[carouselNumber].addEventListener('click', carousel.handleCarouselNext);
            carousel.slider[carouselNumber].addEventListener('transitionend', carousel.handleTransition);
            carousel.slider[carouselNumber].addEventListener('mouseover', carousel.handleMouseOver);
            carousel.slider[carouselNumber].addEventListener('mouseleave', carousel.handleMouseLeave);
    ...
    handleTransition: function(evt) {
        ...
        if (carousel.lastDirection === 'next' && evt.target.className !== "button button__principal") {
